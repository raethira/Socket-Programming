...
stringstream ss;
cout<<"The ServerC recieved input "<<"<"<<word<<">"<<" and operation "<<"<"<<func<<">"<<endl;
if (func =="search")//search operation code piece
{
len = word.size();
ifstream infile;
strcpy(str1Char_array,word.c_str());
infile.open("/home/ee450/Downloads/backendC.txt");
...
count2 = 1;
cout<<"The ServerC has found "<<"<"<<count2<<">"<<" match and "<<"<"<<count<<">"<<" similar words"<<endl;
...
cout<<"The ServerC has found" <<"<"<<n<<">"<<" matches"<<endl;
}
...
//Sending the result to Aws
sendto(sockfd,result,2048,0,(struct sockaddr*)&si_other,addr_size);
cout<<"The ServerC finished sending output to AWS"<<endl;
